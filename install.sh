...
clear
    ...
    hide_output sudo systemctl start mariadb
    hide_output sudo systemctl enable mariadb
    sleep 5
    sudo systemctl status mariadb | sed -n "1,3p"
    echo -e "$GREEN Done...$COL_RESET"
 
    # Installing PHP 8.2 and other necessary packages
    echo
    echo -e "$CYAN => Update system & Install PHP 8.2 & build-essential : $COL_RESET"
    sleep 3
    ...
    if [[ ("$sub_domain" == "n" || "$sub_domain" == "N") ]]; then
        ...
        if [[ ("$ssl_install" == "y" || "$ssl_install" == "Y" || "$ssl_install" == "") ]]; then
            ...
            apt_install certbot python3-certbot-nginx
            sudo certbot --nginx -d "$server_name" -d "$sub_domain.$server_name"
            sudo rm /etc/nginx/sites-available/$server_name.conf
            sudo openssl dhparam -out /etc/ssl/certs/dhparam.pem 2048
